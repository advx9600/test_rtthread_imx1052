...
uint8_t g_macAddr[6] = {0xd4, 0xbe, 0xd9, 0x45, 0x22, 0x69};
//uint8_t g_macAddr[6] = {0xd4, 0xbe, 0xd2, 0x91, 0x33, 0x5a};
...
//	}
    ...
    {
        LOG_E("receive frame > %d ignored", ENET_FRAME_MAX_FRAMELEN);
    }
    else if (length != 0)
    {
        void* recvdata = rt_malloc(length);

        if (recvdata == NULL)
        {
            LOG_E("rt malloc failed");
            return NULL;
        }

        status = ENET_ReadFrame(EXAMPLE_ENET, &g_handle, recvdata, length);
...
//								LOG_D("receive data  %d success\n",length);
            ...
            for (q = p; q != NULL; q = q->next)
            {
                byteslefttocopy = q->len;
                payloadoffset = 0;
                memcpy((uint8_t *)((uint8_t *)q->payload + payloadoffset), (uint8_t *)((uint8_t *)recvdata + bufferoffset), byteslefttocopy);
                bufferoffset = bufferoffset + byteslefttocopy;
            }
...
//								eth_device_ready(&ether_dev);
    ...
    else if (status == kStatus_ENET_RxFrameError)
    {
        ...
        LOG_E("recv error  %d\n", eErrStatic.statsRxLenGreaterErr);
...
{
		uint32_t uid2 = *((int *)0x401f4420);
		int i;
		for (i=0;i<4;i++){
			g_macAddr[2+i] = ((uid2>>8*i) &0xff);
		}
