...
static void print_thread_entry(void *parameter)
{
	...
	while(1){		
		...
		rt_kprintf("\b\b\b\b\b\b%2d.%2d%%",major&0xff,minor&0xff);			
	}
}

static rt_thread_t cpu_thread;
static rt_thread_t print_thread;
void rt_hw_cpu_init(void)
{
    ...
    if(cpu_thread != RT_NULL) 
		...
		print_thread = rt_thread_create("print cpu", print_thread_entry, RT_NULL, 384, 27, 5);
		if(print_thread != RT_NULL) 
        rt_thread_startup(print_thread);
}
void rt_hw_cpu_delete(void)
{
	if (cpu_thread != RT_NULL){
		rt_thread_delete(cpu_thread);
		cpu_thread = RT_NULL;
	}
	if (print_thread != RT_NULL){
		rt_thread_delete(print_thread);
		print_thread = RT_NULL;
	}	
}
